...
TournamentTeamSchema.pre("save", function(next) {
  ...
  if (this.tournamentId?.teamSize && totalMembers !== this.tournamentId.teamSize) {
    next(new Error(`Team must have exactly ${this.tournamentId.teamSize} members`));
  }
...
TournamentTeamSchema.methods.isTeamFull = function() {
  const totalMembers = this.members.length + 1; // +1 for captain
  return totalMembers >= (this.tournamentId?.teamSize || 4);
};
